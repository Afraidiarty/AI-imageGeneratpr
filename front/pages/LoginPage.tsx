...
const LoginPage: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      console.log('✅ Logged in successfully, user:', form.email);
      navigate('/');
    } catch (error) {
      const message =
        error instanceof ApiError ? error.message : 'Failed to login. Please try again.';
  ...
  const handleGoogle = () => {
    // простой и надёжный редирект на backend OAuth
      ...
      <main className="flex items-center justify-center px-4 py-12">
        <div className="w-full max-w-md bg-[#1C1C1E] border border-neutral-800 rounded-2xl shadow-xl p-8">
          ...
          <p className="text-neutral-400 mb-6">
            Продолжите в студию, чтобы генерировать сцены.
          </p>
